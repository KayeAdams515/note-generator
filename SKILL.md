...
- **questions** 设置 1 个问题：
  ...
  - `options`:
    ...
    5. `label`: "视频文件", `description`: "包含课程内容的视频文件（将自动提取音频并转写为文字稿）"
...
1. **询问转写参数**：使用 `AskUserQuestion` 工具依次询问：
   ...
   - 模型大小：
     ...
     - `multiSelect`: false
     - `options`:
       1. `label`: "tiny（最快）", `description`: "适合试运行和设备性能有限的情况"
       2. `label`: "base（推荐）", `description`: "平衡速度与准确度"
       3. `label`: "small", `description`: "较好准确度"
       4. `label`: "medium", `description`: "高准确度"
       5. `label`: "large-v3（最准确）", `description`: "最高准确度，需要较多显存/内存"
...
3. **执行转写**：对用户提供的每个视频文件，使用 Bash 工具运行：
   ```
   python3 <skill_dir>/scripts/transcribe.py --input "<file>" --language <lang> --model <model> --device <device>
   ```
      转写过程中进度日志会每 10% 输出一条，显示当前进度百分比、已处理时长、已识别片段数。
